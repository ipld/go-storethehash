...
func assertCommonPrefixTrimmed(t *testing.T, key1 []byte, key2 []byte, expectedKeyLength int) {
	...
	i, err := OpenIndex(context.Background(), indexPath, primaryStorage, bucketBits, fileSize, 0, 0)
	require.NoError(t, err)
...
func TestIndexGetEmptyIndex(t *testing.T) {
	...
	tempDir := t.TempDir()
	indexPath := filepath.Join(tempDir, "storethehash.index")
	index, err := OpenIndex(context.Background(), indexPath, primaryStorage, bucketBits, fileSize, 0, 0)
	require.NoError(t, err)
...
func TestIndexGet(t *testing.T) {
	...
	i2, err := OpenIndex(context.Background(), indexPath, primaryStorage, bucketBits, fileSize, 0, 0)
	require.NoError(t, err)
...
func TestIndexHeader(t *testing.T) {
	...
	primaryStorage := inmemory.NewInmemory([][2][]byte{})
	i1, err := OpenIndex(context.Background(), indexPath, primaryStorage, bucketBits, fileSize, 0, 0)
	require.NoError(t, err)
	...
	i2, err := OpenIndex(context.Background(), indexPath, inmemory.NewInmemory([][2][]byte{}), bucketBits, fileSize, 0, 0)
	require.NoError(t, err)
	t.Cleanup(func() { i2.Close() })
...
func TestIndexGetBad(t *testing.T) {
	...
	tempDir := t.TempDir()
	indexPath := filepath.Join(tempDir, "storethehash.index")
	i, err := OpenIndex(context.Background(), indexPath, primaryStorage, bucketBits, 0, 0, 0)
