...
func assertCommonPrefixTrimmed(t *testing.T, key1 []byte, key2 []byte, expectedKeyLength int) {
	...
	i, err := OpenIndex(context.Background(), indexPath, primaryStorage, bucketBits, fileSize)
	require.NoError(t, err)
...
func TestIndexGetEmptyIndex(t *testing.T) {
	...
	tempDir := t.TempDir()
	indexPath := filepath.Join(tempDir, "storethehash.index")
	index, err := OpenIndex(context.Background(), indexPath, primaryStorage, bucketBits, fileSize)
	require.NoError(t, err)
...
func TestIndexHeader(t *testing.T) {
	...
	primaryStorage := inmemory.NewInmemory([][2][]byte{})
	i1, err := OpenIndex(context.Background(), indexPath, primaryStorage, bucketBits, fileSize)
	require.NoError(t, err)
	...
	i2, err := OpenIndex(context.Background(), indexPath, inmemory.NewInmemory([][2][]byte{}), bucketBits, fileSize)
	require.NoError(t, err)
...
func TestIndexGetBad(t *testing.T) {
	...
	tempDir := t.TempDir()
	indexPath := filepath.Join(tempDir, "storethehash.index")
	i, err := OpenIndex(context.Background(), indexPath, primaryStorage, bucketBits, 0)
