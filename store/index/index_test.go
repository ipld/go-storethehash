...
func assertCommonPrefixTrimmed(t *testing.T, key1 []byte, key2 []byte, expectedKeyLength int) {
	...
	i, err := Open(context.Background(), indexPath, primaryStorage, bucketBits, fileSize)
	require.NoError(t, err)
	...
	iter := NewIter(i.basePath, i.fileNum)
	defer iter.Close()
...
func TestIndexGetEmptyIndex(t *testing.T) {
	...
	tempDir := t.TempDir()
	indexPath := filepath.Join(tempDir, "storethehash.index")
	index, err := Open(context.Background(), indexPath, primaryStorage, bucketBits, fileSize)
	require.NoError(t, err)
...
func TestIndexHeader(t *testing.T) {
	...
	primaryStorage := inmemory.NewInmemory([][2][]byte{})
	i1, err := Open(context.Background(), indexPath, primaryStorage, bucketBits, fileSize)
	require.NoError(t, err)
...
func TestIndexGetBad(t *testing.T) {
	...
	tempDir := t.TempDir()
	indexPath := filepath.Join(tempDir, "storethehash.index")
	i, err := Open(context.Background(), indexPath, primaryStorage, bucketBits, 0)
